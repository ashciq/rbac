#!/usr/bin/env python3
"""
RBAC Framework v3 Demo
Demonstrates: 11 actions, composite roles, permission dependencies,
admin overrides, org hierarchy, and UI-exportable permission manifests.
"""
...
def main():
    ...
    for uid, user in users.items():
        ...
        exclusion_keys = {f"{a}:{r}" for r, acts in user.exclusions.items() for a in acts}
        grant_keys = {f"{a}:{r}" for r, acts in user.grants.items() for a in acts}

        roles_str = ", ".join(user.roles)
        extras = []
        if grant_keys:
            extras.append(f"+{','.join(sorted(grant_keys))}")
        if exclusion_keys:
            extras.append(f"-{','.join(sorted(exclusion_keys))}")
    ...
    explain_cases = [
        ...
        ("dave", "delete", "orders"),
        ("eve", "create", "orders"),
        ("frank", "download", "audit_logs"),
        ("grace", "upload", "documents"),
        ("grace", "approve", "orders"),
        ("bob", "approve", "invoices"),
    ]
    ...
    header("COMPOSITE ROLE DEMO: grace (contributor + approver)")
    grace = engine.get_user("grace")
    effective = engine.get_effective_permissions(grace)
    print(f"\n  Roles: {', '.join(grace.roles)}")
    print(f"  Extra grants: {dict({r: list(a) for r, a in grace.grants.items()})}")
    print(f"  Exclusions: {dict({r: list(a) for r, a in grace.exclusions.items()})}")
